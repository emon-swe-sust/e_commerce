...
function Products () {
  ...
  return (
    <div>
    <div className="my-3 flex bg-gradient-to-r from-indigo-300 to-green-300 text-white
    shadow-xl">
      ...
      {itemList.length > 0 ? (
        <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 px-0 sm:px-20 md:px-20 xl:px-60'>
          {itemList.map(({ title, id,  price, description, category,
           image, rating}) => {
            return(
                <ItemCards key={id} title={title} id={id} price={price} description={description}
